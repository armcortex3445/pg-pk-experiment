...
export default function () {
  ...
  const rows = db.query(`SELECT * FROM ${tableName} WHERE id = $1;`, targetId);
  // check(rows, {
  //   'is equal' : (rows) => rows.every(row => Number.isInteger(targetId) ? row.id === targetId : String.fromCharCode(...row.id) === targetId) ,
  //   'is unique row ': (r) => r.length === 1,
  // });
