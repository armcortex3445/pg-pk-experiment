...
export async function inspectTableCorrelation({tableName,indexColum}){
  ...
  try{
    const res = await client.query(
      `
        SELECT tablename, attname, round(correlation::numeric,5) AS correlation
        FROM pg_stats 
        WHERE tablename  = '${tableName}' AND attname = '${indexColum}';
      `
    );
...
export async function runCluster({tableName, indexName}) {
  if(!pool){
    throw Error(`pool is not created`);
  }
  ...
  try{
    ...
    await client.query(`CLUSTER ${tableName} USING ${indexName};`);

    // need to analyze table statistic because CLUSTER command doesn't automatically update statistic 
    // ref : https://www.postgresql.org/docs/17/sql-cluster.html
